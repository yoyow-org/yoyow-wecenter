...
class ajax extends AWS_CONTROLLER
{
    ...
    public function modify_question_action()
    {
        ...
        {
             $_POST['question_content'] = filter_Emoji($_POST['question_content']);
        }
    ...
    public function publish_question_action()
    {
        ...
        $user_question = $this->model("question")->get_last_question_by_uid($this->user_id);
        if ($user_question && $user_question['question_content'] == htmlspecialchars($_POST['question_content']) && $user_question['question_detail'] == htmlspecialchars($_POST['question_detail'])){
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('发起问题的内容和标题与上一次提交内容重复，请重新填写')));
        }
        ...
        if ($this->user_id && HTTP::get_cookie('yoyow_add_draft_'.$add_draft_type.'_'.$this->user_id))
        {   
            HTTP::set_cookie('yoyow_add_draft_'.$add_draft_type.'_'.$this->user_id,'',0);
        } 
    ...
    public function publish_article_action()
    {
        ...
        {
            $_POST['title'] = filter_Emoji($_POST['title']);
        }
        ...
        $user_atricle = $this->model("article")->get_last_article_by_uid($this->user_id);

        if ($user_atricle && $user_atricle['title'] == htmlspecialchars($_POST['title']) && $user_atricle['message'] == htmlspecialchars($_POST['message'])   ){
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('发起文章的内容和标题与上一次提交内容重复，请重新填写')));
        }
        ...
        if ($this->publish_approval_valid(array($_POST['title'],$_POST['message'])))
        {
    ...
    public function modify_article_action()
    {
        ...
        if ($_FILES['article_cover_file'])
        {
            ...
            $this->article_cover_upload_action(intval($_POST['article_id']),'article_cover_file',$max_size);
        }
     ...
     * @param $max_size
     */
    public function article_cover_upload_action($article_id,$column_name,$max_size)
    {
        AWS_APP::upload()->initialize(array(
            'allowed_types' => get_setting('allowed_upload_types'),
            'upload_path' => get_setting('upload_dir') . '/article/cover/' . $this->model('article')->get_cover($article_id, '', 1),
            'is_image' => TRUE,
            'max_size' =>$max_size,
            'file_name' => $this->model('article')->get_cover($article_id, '', 2),
            'encrypt_name' => FALSE
        ...
        if ($upload_data['is_image'] == 1){
            ...
            foreach (AWS_APP::config()->get('image')->article_cover_thumbnail AS $key => $val) {
                $thumb_file[$key] = $upload_data['file_path'] . $this->model('article')->get_cover($article_id, $key, 2);
        ...
        $image_info = @getimagesize(get_setting('upload_dir') . '/article/cover/' . $this->model('article')->get_cover($article_id, 'real', 3));

        if($image_info[2] == 6)
        {
            $update_data['cover_file'] = $this->model('article')->get_cover($article_id, 'real', 3);

        }else
        {
            $update_data['cover_file'] = $this->model('article')->get_cover($article_id, null, 1) . basename($thumb_file['min']);
        } 
