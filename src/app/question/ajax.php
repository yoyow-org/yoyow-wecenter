...
class ajax extends AWS_CONTROLLER
{
	...
	public function fetch_answer_data_action()
	{
		...
		{
            $answer_info_data= $answer_info['answer_content'];
            $answer_info_data=htmlspecialchars_decode(nl2br($answer_info_data));
            echo json_encode($answer_info_data);
		}
	}
	...
	public function save_answer_comment_action()
	{
        ...
        $user_answer_comment = $this->model('answer')->get_last_one_answer_comment_by_uid_and_answer_id($this->user_id,$_GET['answer_id']);
        if (count($user_answer_comment) > 0 && $user_answer_comment == htmlspecialchars($_POST['message'])){
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('回复内容与上一次回复内容重复，请重新填写')));
        }
	...
	public function answer_vote_action()
	{
        ...
        //对点赞 踩进行10秒限制
        $vote_flag = AWS_APP::cache()->get('answer_vote_'.$this->user_id);
        $vote_user_info = $this->model('account')->get_user_info_by_uid($this->user_id);
		if($_POST['value']==-1 && ($vote_user_info['reputation']<10)){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('没有足够权限')));
        }
        if ((!$vote_info = $this->model('answer')->get_answer_vote_status($_POST['answer_id'], $this->user_id)) && $vote_flag ) //添加记录
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请不要短时间内频繁点赞或踩')));
        }else if (($vote_info['vote_value'] != $_POST['value']) && $vote_flag){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请不要短时间内频繁点赞或踩')));
        }
        if (!$vote_info = $this->model('answer')->get_answer_vote_status($_POST['answer_id'], $this->user_id)) //添加记录
        {
            AWS_APP::cache()->set('answer_vote_'.$this->user_id, 11, get_setting('integral_system_config_vote_time_interval'));
        }else if ($vote_info['vote_value'] != $_POST['value']){
            AWS_APP::cache()->set('answer_vote_'.$this->user_id, 11, get_setting('integral_system_config_vote_time_interval'));
	...
	public function save_answer_action()
	{
        ...
        //同一用户回答的内容不能与之前回答的最后一次内容相同
        $user_answer = $this->model('answer')->get_last_one_answer_by_uid_and_question_id($this->user_id,$question_info['question_id']);
        if (count($user_answer) > 0 && $user_answer == htmlspecialchars($answer_content)){
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('回答内容与上一次回答内容重复，请重新填写')));
        }
...
//				), 1, null));
                H::ajax_json_output(AWS_APP::RSM(array(
                    'url' => get_js_url('/question/'.$_POST['question_id'])
                ), 1, AWS_APP::lang()->_t('提交成功')));
			}
	...
	public function remove_question_action()
	{
		...
		if ($question_info = $this->model('question')->get_question_info_by_id($_POST['question_id']))
		{
			...
			$this->model('question')->remove_question($question_info['question_id'],$question_info['published_uid']);
		}
	...
	public function set_recommend_action()
	{
        ...
        $uid=$this->model('question')->get_question_uid($_POST['question_id']);
        switch ($_POST['action'])
        {
            case 'set':
                $this->model('question')->set_recommend($_POST['question_id'],$uid);
                break;

            case 'unset':
                $this->model('question')->unset_recommend($_POST['question_id'],$uid);
                break;
        }
	...
	public function set_top_question_action()
    {
        if (!$this->user_info['permission']['is_administortar'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('对不起, 你没有设置置顶的权限')));
        }
        if($this->model('question')->select_set_top_num()[0]['count(*)']>=3 && $_POST['action'] == 'set'){
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('对不起, 你只有设置置顶三条的权限')));
        }
        switch ($_POST['action'])
        {
            case 'set':
                $this->model('question')->set_top($_POST['question_id']);
                break;

            case 'unset':
                $this->model('question')->unset_top($_POST['question_id']);
                break;
        }
    ...
    public function set_hide_top_question_action()
    {
        if (!$this->user_info['permission']['is_administortar'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('对不起, 你没有设置置顶的权限')));
        }
        // if($this->model('question')->select_set_hide_top_num()[0]['count(*)']>=3 && $_POST['action'] == 'set'){
        //     H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('对不起, 你只有设置置顶三条的权限')));
        // }
        switch ($_POST['action'])
        {
            case 'set':
                $this->model('question')->set_hide_top($_POST['question_id']);
                break;

            case 'unset':
                $this->model('question')->unset_hide_top($_POST['question_id']);
                break;
        }
