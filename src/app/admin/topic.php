	...
	public function list_action()
	{
		...
		if ($topic_list)
		{
			foreach ($topic_list AS $key => $topic_info)
			{
				...
				$topic_add_uid = $this->model('topic')->get_add_topic_user_info($topic_info['topic_id']);

                $topic_list[$key]['add_info'] = $this->model('account')->get_user_info_by_uid($topic_add_uid);
