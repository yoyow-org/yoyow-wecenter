...
class ajax_yoyow extends AWS_CONTROLLER
{
    ...
    public function setup()
    {
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('本站未开通 YOYOW 登录')));
        }
    }
    ...
    public function register_action()
    {
        ...
        if ($this->model('account')->check_email($_POST['email'])) {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('E-Mail已被使用')));
        }
        if(!H::valid_email($_POST['email']))
        {
            H::ajax_json_output(AWS_APP::RSM(array(
                'input' => 'email'
            ), -1, AWS_APP::lang()->_t('E-Mail格式不正确')));
        }
...
//        if($_POST['password']==""){
//            $_POST['password']=$_POST['yoyow_name'];
//        }
        ...
        if (!$uid)
        {
            ...
            $this->model('openid_yoyow')->bind_account($_POST['yoyow_name'],$uid, true);
        }
        if (get_setting('register_valid_type') == 'approval')
        {
