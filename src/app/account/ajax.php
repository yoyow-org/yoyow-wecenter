...
class ajax extends AWS_CONTROLLER
{
    ...
    public function seccode_verify_action()
    {
    ...
    public function register_process_action()
    {
        ...
        if ($this->model('account')->check_mobile($_POST['mobile'])) {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('手机号 已经被使用, 或格式不正确')));
        }
        if ($_POST['smscode']=='') {
        ...
        if(strlen( $_POST['inviteCode'])> 0){
            if(!$invite = $this->model('invitation')->check_invite_code($_POST['inviteCode'])){
                H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('邀请码不存在或失效，请确认')));
            }
        }
...
//            $this->model('follow')->user_follow_add($follow_users['uid'], $uid);
            $this->model('integral')->process($follow_users['uid'], 'INVITE', get_setting('integral_system_config_invite'), '邀请注册: ' . $_POST['mobile'], $follow_users['uid']);
        }
        ...
        if (strlen($_POST['inviteCode'])>0) {
            ...
            $first_invite_info = $this->model('invitation')->select_invite_by_active_id($uid);
            $base = (get_setting('register_reward_amount')=='') ? 0 : get_setting('register_reward_amount');
            $first_percent=(get_setting('register_reward_first_percent')=='') ? 0 : get_setting('register_reward_first_percent');
            $second_percent=(get_setting('register_reward_second_percent')=='') ? 0 : get_setting('register_reward_second_percent');
            $first_user_name = $this->model('account')->fetch_one('users','user_name','uid = '.$invite['uid']);
            $this->model('invitation')->add_invitation_register_yoyow($first_invite_info['invitation_id'],substr(sprintf( '%.6f' , $base* $first_percent), 0, -2),1,$invite['uid'],$uid,$resiter_user_name);
            $this->model('invitation')->add_invitation_register_yoyow($first_invite_info['invitation_id'],substr(sprintf( '%.6f' , $base), 0, -2),0,$uid,$uid);
            if($second_invite_info = $this->model('invitation')->select_invite_by_active_id($invite['uid'])){
                $this->model('invitation')->add_invitation_register_yoyow($first_invite_info['invitation_id'],substr(sprintf( '%.6f' , $base* $second_percent), 0, -2),2,$second_invite_info['uid'],$uid,$first_user_name,$resiter_user_name);
            }
            $register_user =$this->model('account')->get_user_info_by_uid($uid);
            $p_urls= get_js_url('/people/' . $register_user['url_token']);
            if($second_invite_info){
                $first_user= $this->model('account')->get_user_info_by_uid($invite['uid']);
                $p_url= get_js_url('/people/' . $first_user['url_token']);
                //注册后，将奖励结果推送给上级用户
                $this->model('notify')->send(
                    null,
                    $second_invite_info['uid'],
                    notify_class::TYPE_CONTEXT,
                    notify_class::INVITE_YOYOW_COIN,
                    $uid,
                    array('content' => '<a href="' . $p_urls . '">' . $register_user['user_name'] . '</a>'.'通过'. '<a href="' . $p_url . '">' . $first_user['user_name'] . '</a>' . '的邀请链接注册，你将获得'.substr(sprintf( '%.6f' , $base* $second_percent), 0, -2).'yoyow奖励')
            ...
            $this->model('notify')->send(
                ...
                notify_class::INVITE_YOYOW_COIN,
                $uid,
                array('content' => '<a href="' . $p_urls . '">' . $register_user['user_name'] . '</a>'.'通过你的邀请链接注册，你将获得'.substr(sprintf( '%.6f' , $base* $first_percent), 0, -2).'yoyow奖励')
            );
        }else{
            $base = (get_setting('register_reward_amount')=='') ? 0 : get_setting('register_reward_amount');
            $this->model('invitation')->add_invitation_register_yoyow('',substr(sprintf( '%.6f' , $base), 0, -2),0,$uid,$uid);
        }
        ...
        if ($_POST['_is_mobile']) {
            ...
            if ($_POST['return_url']) {
                ...
                $return_url = get_js_url('/m/first_login-TRUE');
            }
    ...
    public function login_process_action()
    {
        ...
        if (! $user_info)
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请输入正确的账号或密码')));
        }
        else
        {
            ...
            if($_POST['yoyow_name']){
                // 判断当前账号是否已绑定yoyow
                if($this->model('openid_yoyow')->get_yoyow_user_by_uid($user_info['uid'])){
                    H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('抱歉, 你的账号已经绑定yoyow其他账号')));
                }

                $this->model('openid_yoyow')->bind_account($_POST['yoyow_name'],$user_info['uid'], true);

                /**************** 20180426-20180430活动 绑定yoyow账号送10个yoyow币 jiangchengkai  2018-04-19 start *************************/
                if($this->model('account')->get_register_send_yoyow_record($user_info['uid'], 1) &&
                    $this->model('account')->is_send_yoyow()) {
                    $this->model('account')->register_send_yoyow($user_info['uid'], 2, get_setting('register_bind_reward'));
                }
...
//            if(!$user = $this->model('openid_yoyow')->get_yoyow_user_by_uid($user_info['uid'])){
//                H::ajax_json_output(AWS_APP::RSM(array(
//                    'url' => get_js_url('/account/openid/yoyow/login/uid-'.$user_info['uid'])
//                ), 1, null));
//            }
            ...
            else
            {
                ...
                $this->handle_integral($user_info);
                ...
                else if ($_POST['return_url'] AND !strstr($_POST['return_url'], '/logout') AND $user_info['is_first_login'] AND
                    ($_POST['_is_mobile'] AND strstr($_POST['return_url'], '/m/') OR
                        strstr($_POST['return_url'], '://') AND strstr($_POST['return_url'], base_url())))
                {
                    $url = get_js_url($_POST['return_url'].'first_login-TRUE');
                }
                else if ($_POST['return_url'] AND !strstr($_POST['return_url'], '/logout') AND
                    ($_POST['_is_mobile'] AND strstr($_POST['return_url'], '/m/') OR
                        strstr($_POST['return_url'], '://') AND strstr($_POST['return_url'], base_url())))
                {
                    if(strstr($_POST['return_url'], '/account/find_password/next_find_password')){
                        $url = get_js_url(base_url());
                    }else{
            ...
            if($_POST['yoyow_name']){
                setcookie('yoyow_message_'.$user_info['uid'],$_POST['yoyow_name'], time()+3600*24, "/");
                $url = '/';
            }
     ...
     */
    private function handle_integral($user_info)
    {
        if (get_setting('integral_system_config_login_enabled')!='Y') {
            return;
        }

        if ($log = $this->model('integral')->fetch_last_log($user_info['uid'], 'LOGIN') and date('Ymd', $log) == date('Ymd')) {
            return;
        }
        $lastLoginTime = date('Ymd', $user_info['last_login']);//最后登录日期
        $login_integral_base = get_setting('integral_system_config_login_base');//登录奖励基础积分
        $login_integral_cycle = get_setting('integral_system_config_login_cycle');//登录奖励周期天数
        $login_coefficient = get_setting('integral_system_config_login_coefficient');//登陆奖励积分系数
        $allow_login_interrupt = get_setting('integral_system_config_allow_login_interrupt');//是否允许中断连续登录
        $login_continuous = $user_info['login_continuous']+1;//用户已连续登录天数
//        判断最后一次是不是昨天登陆
        if ($lastLoginTime == date("Ymd", strtotime("-1 day"))) {
            $score = $login_integral_base;
            if ($login_continuous % $login_integral_cycle == 1) {
                $score += intval($login_continuous / $login_integral_cycle) * $login_coefficient * $login_integral_base;
            }
        } else {
            //判断是否允许连续登录
            if ($allow_login_interrupt == 'Y') {
                $score = $login_integral_base;
                if ($login_continuous % $login_integral_cycle == 1) {
                    $score += intval($login_continuous / $login_integral_cycle) * $login_coefficient * $login_integral_base;
                }
                ...
                $login_continuous = 1;
                $score = intval($login_continuous * $login_integral_base);
            }
        }
        //更新连续登录天数
        $this->model('account')->update_users_fields(['login_continuous' => $login_continuous], $user_info['uid']);
        //插入积分记录
        $this->model('integral')->process($user_info['uid'], 'LOGIN', $score, '用户' . $user_info['uid'] . '连续登录' . $login_continuous. '天获得积分' . $score);
    }
    ...
    public function download_app_template_action()
    {
        TPL::output('account/ajax/download_app_template');
    }
    ...
    public function save_add_draft_action()
    {
    ...
    public function delete_add_draft_action()
    {
        if (!$_POST['type'] AND !$this->user_id) {
            die;
        }
    ...
    public function profile_setting_action()
    {
        ...
        $old_user_info = $this->model('account')->get_user_info_by_uid($this->user_id);
        if(empty($old_user_info['mobile'])){
    ...
    //yoyow账号注册登录
    public function yoyow_login_process_action()
    {
        if ($yoyow = $this->model('openid_yoyow')->yoyow_login_process(session_id())) {
            // 根据yoyow获取用户uid
            if($user_info = $this->model('openid_yoyow')->get_yoyow_user_by_name($yoyow, null))
            {
                //若yoyow获取到的信息没有用户uid，则进入注册页面
                if(!$user_info['uid'])
                {
                    H::ajax_json_output(AWS_APP::RSM($yoyow, 2, null));
                }else{
                    $user = $this->model('account')->get_user_info_by_uid($user_info['uid']);
                    $this->model('account')->setcookie_login($user['uid'], $user['user_name'], $user['password'], $user['salt'], null, false);
                    H::ajax_json_output(AWS_APP::RSM(null, 1, null));
                }
            }else{
                //根据yoyow查找不到yoyow用户表中的yoyow绑定记录，则插入绑定记录
                $this->model('openid_yoyow')->bind_account($yoyow);
                H::ajax_json_output(AWS_APP::RSM($yoyow, 2, null));
            }
    ...
    {
        if ($this->user_info['email']) {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('当前账号已经完善资料')));
        }
    ...
    public function first_find_password_action()
    {
        ...
        if (is_mobile()) {
            ...
            $url = get_js_url('/account/find_password/next_find_password/mobile-'.$_POST['mobile']);
        }
