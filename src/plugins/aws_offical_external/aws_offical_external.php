	...
	public function format_js_question_output($data)
	{
		if ($data)
		{
			foreach ($data AS $key => $val)
			{
				...
				if(get_setting('view_count_times')=='0' || get_setting('view_count_times')== ''){
				    $view_count=1;
                }else{
				    $view_count=get_setting('view_count_times');
                }

				$output .= ' • ' . date_friendly($val['update_time']) . ' • ' . $val['focus_count'] . ' 人关注 • ' . ($val['view_count'] * $view_count) . ' 次浏览</p></div>';
			}
		}
