...
function FileUpload(type, element, container, url, options, callback) {
    ...
    this.options = {
        ...
        '</div>' +
        '</li>',
        'deleteBtnTemplate': '<a class="delete-file">'+_t('删除')+'</a>',
        'insertBtnTemplate': '<a class="insert-file">'+_t('插入')+'</a>',
        'tag': '',
        'accept':''
    };
    if (options.editor) {
...
FileUpload.prototype =
    {
        ...
        init: function (element, container) {
            ...
            if (navigator.userAgent.indexOf("MSIE 8.0") > 0) {
                ...
                var $input = $('#upload-form-' + this.options.tag + ' .file-input')
        ...
        createForm: function () {
            ...
            $(form).attr({
                'id': 'upload-form-' + this.options.tag,
                'class': 'upload-form',
                'action': this.url,
                'target': 'ajaxUpload'
            });
        ...
        createHiddenInput: function (attach_id) {
            var _this = this,
                input = this.toElement('<input type="hidden" name="attach_ids[]" class="hidden-input" />');
        ...
        getIframeContentJSON: function (iframe, container) {
            ...
            if (doc.body.innerHTML) {
                ...
                if (this.type == 'file') {
                    ...
                    filename = this.getName($('#upload-form-' + this.options.tag + ' .file-input')[0].value);
