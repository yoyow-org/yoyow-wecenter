...
AWS.User =
{
	...
	// 删除草稿
	delete_add_draft: function(type)
	{   
		if(type)
		{
			$.post(G_BASE_URL + '/account/ajax/delete_add_draft/', 'type=' + type, function (result)
			{
				if (result.errno != 1)
				{
					AWS.alert(result.err);
		...
		$.post(G_BASE_URL + '/question/ajax/answer_vote/', 'answer_id=' + answer_id + '&value=1', function(result){
			if(result.errno == -1){
                ...
                if ($(selector).parents('.aw-item').find('.aw-agree-by').text().match(user_name))
                {
                    ...
                    {
                        $(selector).parents('.operate').find('.agree').find('.count').html(parseInt($(selector).parents('.operate').find('.agree').find('.count').text()) - 1);
                    }
                ...
                else
                {
                    ...
                    if ($(selector).parents('.operate').find('.disagree').hasClass('active')) {
                        ...
                        $(selector).parents('.operate').find('.disagree').find('.count').html(parseInt($(selector).parents('.operate').find('.disagree').find('.count').text()) - 1);
                    }
                    $(selector).parents('.operate').find('.agree').find('.count').html(parseInt($(selector).parents('.operate').find('.agree').find('.count').text()) + 1);
	...
	init_topic_edit_box: function(selector) //selector -> .aw-edit-topic
	{
		$(selector).click(function ()
		{
			...
			if (_topic_editor.find('.aw-edit-topic-box').length == 0)
			{
				...
				{
					var value = _topic_editor.find('#aw_edit_topic_title').val();
					...
					if (value != '')
					{
						switch (data_type)
						{
							case 'publish':
                                var arr = new Array();
                                if(_topic_editor.find('.tag-bar').children("span").length > 1 && $.trim(value)){
                                	var i = 0;
									$(_topic_editor.find('.tag-bar').children("span").each(function(){
										 arr[i] = $.trim($(this).children(".text").html());
										 i++;
									}));
								}
                                if($.trim(value) && $.inArray($.trim(value),arr) == -1){
