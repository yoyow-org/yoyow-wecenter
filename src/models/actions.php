...
class actions_class extends AWS_MODEL
{
	public function home_activity($uid, $limit = 10)
	{
		...
		if ($user_follow_uids = $this->model('follow')->get_user_friends_ids($uid))
		{
			...
			// 增加赞同, 文章评论
			$where_in[] = "(uid IN (" . implode(',', $user_follow_uids) . ") AND associate_action IN(" . ACTION_LOG::ADD_AGREE_ARTICLE .", " . ACTION_LOG::ADD_COMMENT_ARTICLE . ", " . ACTION_LOG::ADD_AGREE . ") AND uid <> " . $uid . ")";
		...
		foreach ($action_list as $key => $val)
		{
			...
			)))
			{
				...
				if($val['associate_action'] == ACTION_LOG::ADD_ARTICLE OR $val['associate_action'] == ACTION_LOG::ADD_AGREE_ARTICLE ){
                    $action_list_article_new_comment_ids[] = $val['associate_id'];
                }
			}
			...
			else
			{
                ...
                {
                   /* if($this->model('question')->get_question_comments_id_by_question_id($val['associate_id'])){
                        $action_list_answer_ids[] =  $this->model('question')->get_question_comments_id_by_question_id($val['associate_id']);
                    }*/

                    $action_list_answer_new_ids[] = $val['associate_id'];
                }
			}
		...
		if($action_list_answer_new_ids){
            $question_new_comments = $this->model('question')->get_question_comments_new_by_ids($action_list_answer_new_ids);
        }
		...
		if($action_list_article_new_comment_ids){
            $article_new_comment = $this->model('article')->get_comments_new_by_ids($action_list_article_new_comment_ids);
        }
		...
		foreach ($action_list as $key => $val)
		{
			...
			switch ($val['associate_action'])
			{
				...
				case ACTION_LOG::ADD_AGREE_ARTICLE:
                    ...
                    $action_list[$key]['message'] = $article_info['message'];
                    $action_list[$key]['article_info'] = $article_info;
                    $action_list[$key]['comment_new_info'] = $article_new_comment[$val['associate_id']];
                    if($action_list[$key]['comment_new_info']){
                        $action_list[$key]['comment_new_info']['user_info'] = $this->model('account')->get_user_info_by_uid($action_list[$key]['comment_new_info']['uid']);
                    }
                ...
                case ACTION_LOG::ADD_REQUESTION_FOCUS:
                     ...
                     $action_list[$key]['message'] = $question_info['question_detail'];
                     ...
                     $action_list[$key]['answer_new_info'] = $question_new_comments[$val['associate_id']];
                    if($action_list[$key]['answer_new_info']){
                        $action_list[$key]['answer_new_info']['user_info'] = $this->model('account')->get_user_info_by_uid($action_list[$key]['answer_new_info']['uid']);
                    }
                     $action_list[$key]['question_info'] = $question_info;
                    // 处理回复
                     if ($action_list[$key]['answer_new_info']['answer_id'])
                     {
                         if ($action_list[$key]['answer_new_info']['anonymous'] AND $val['associate_action'] == ACTION_LOG::ANSWER_QUESTION)
                         {
                         ...
                         $final_new_list_answer_ids[] = $action_list[$key]['answer_new_info']['answer_id'];

                         if ($action_list[$key]['answer_new_info']['has_attach'])
                         {
                               $action_list[$key]['answer_new_info']['attachs'] = $answer_attachs[$action_list[$key]['answer_info']['answer_id']];
                         }
                    }
        ...
        if ($final_new_list_answer_ids)
        {
            $answer_new_agree_users = $this->model('answer')->get_vote_user_by_answer_ids($final_new_list_answer_ids);
            $answer_new_vote_status = $this->model('answer')->get_answer_vote_status($final_new_list_answer_ids, $uid);

            $answer_new_users_rated_thanks = $this->model('answer')->users_rated('thanks', $final_new_list_answer_ids, $uid);
            $answer_new_users_rated_uninterested = $this->model('answer')->users_rated('uninterested', $final_new_list_answer_ids, $uid);
        }
		...
		foreach ($action_list as $key => $val)
		{
            ...
            if (isset($action_list[$key]['answer_new_info']['answer_id']))
            {
                $answer_id = $action_list[$key]['answer_new_info']['answer_id'];

                if (isset($answer_new_agree_users[$answer_id]))
                {

                    $action_list[$key]['answer_new_info']['agree_users'] = $answer_new_agree_users[$answer_id];
                }

                if (isset($answer_new_vote_status[$answer_id]))
                {
                    $action_list[$key]['answer_new_info']['agree_status'] = $answer_new_vote_status[$answer_id];
                }

                if (isset($answer_new_users_rated_thanks[$answer_id]))
                {
                    $action_list[$key]['answer_new_info']['user_rated_thanks'] = $answer_new_users_rated_thanks[$answer_id];
                }

                if (isset($answer_new_users_rated_uninterested[$answer_id]))
                {
                    $action_list[$key]['answer_new_info']['user_rated_uninterested'] = $answer_new_users_rated_uninterested[$answer_id];
                }
               // var_dump($action_list[$key]['answer_new_info']['agree_users']);die;
            }
		}
