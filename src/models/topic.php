...
class topic_class extends AWS_MODEL
{
	...
	public function get_add_topic_user_info($topic_id){
	    $prefix=AWS_APP::config()->get('database')->prefix;
	    $sql="SELECT uid FROM ".$prefix."user_action_history WHERE associate_id = ".$topic_id." AND associate_action = 401 AND associate_attached = - 1";
	    $uid = $this->query_all($sql);
	    return $uid[0]['uid'];
    }
}
