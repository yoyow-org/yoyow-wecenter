...
class notify_class extends AWS_MODEL
{
	...
	const YOYOW_COIN	= 10;	// yoyow

    const INVITE_YOYOW_COIN	= 11;	// 邀请yoyow
	...
	const TYPE_COIN_DISTRIBUTE = 143; // 用户被分币
	...
	public function list_notification($recipient_uid, $read_status = 0, $limit = null)
	{
		...
		foreach ($notify_list as $key => $notify)
		{
			...
			switch ($notify['model_type'])
			{
                ...
                case self::YOYOW_COIN:
                    $tmp_data['coin'] = $coin_yoyow[$key];
                    break;
                case self::INVITE_YOYOW_COIN:
                    $tmp_data['content'] = $data['content'];
		...
		foreach ($extends as $action_type => $val)
		{
			...
			foreach ($action_users as $uid => $action)
			{
				...
				if ($answer_ids)
				{
					...
					$querys[] = 'item_id=' . implode(',', $answer_ids) . '&answer_id=' . implode(',', $answer_ids) .'&single=TRUE'.'#!answer_' . array_pop($answer_ids);
				}
		...
		foreach ($data AS $key => $val)
		{
			...
			else
			{
				switch ($val['action_type'])
				{
					...
					case self::TYPE_COIN_DISTRIBUTE:
						$data[$key]['message'] = AWS_APP::lang()->_t('%s0',
                            array(
                            '<span>平台向您发送 <span style="color: #155faa; font-size:16px">' .$val['coin'] .'</span> yoyow币</span>'
                            )
                            );
