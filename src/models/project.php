...
class project_class extends AWS_MODEL
{
    ...
    public function get_register_bind_list($page = null, $limit = 10){
        $sql = "SELECT ryr.id,ryr.uid,u.user_name,ryr.operate,ryr.coin,ryr.ope_time,ryr.status FROM ".$this->get_table('register_yoyow_record') ." as ryr inner join ".$this->get_table('users')." as u on ryr.uid = u.uid ";

        $sql.= " ORDER BY ryr.ope_time DESC LIMIT ". (($page-1)*$limit).",".$limit;
    ...
    public function get_reputation_users_list($page = null, $limit = 10){
        $sql = "select * from ".get_table('reputation_users')." LIMIT ".(($page-1)*$limit).",".$limit;
