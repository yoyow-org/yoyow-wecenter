...
class posts_class extends AWS_MODEL
{
	public function set_posts_index($post_id, $post_type, $data = null)
	{
		...
		else
		{
			switch ($post_type)
			{
                case 'set_top_question':
                case 'set_hide_top_question':
                case 'unset_top_question':
                case 'unset_hide_top_question':
				case 'question':
	...
	{
		$order_key = 'set_top_time DESC,set_hide_top_time DESC,add_time DESC';
		...
		switch ($sort)
		{
			...
			case 'new' :
				$order_key = 'set_top_time DESC,set_hide_top_time DESC,update_time DESC';
		...
		{
			$where = array();
			$where [] = "post_type != 'article'";//发现不要文章  新开文章专区
	...
	public function get_hot_posts($post_type, $category_id = 0, $topic_ids = null, $day = 30, $page = 1, $per_page = 10)
	{
		...
		$posts_index = $this->fetch_page('posts_index', implode(' AND ', $where), 'set_top_time DESC,set_hide_top_time DESC,popular_value DESC', $page, $per_page);
