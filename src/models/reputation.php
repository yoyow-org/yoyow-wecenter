...
class reputation_class extends AWS_MODEL
{
	...
	{
		if (!$user_info = $this->model('account')->get_reputation_user_info_by_uid($uid))
		{
			return false;
		}
        ...
        // 计算威望
        $user_reputation=$user_reputation + $user_info['reputation_extend'];
        $new_praise_no_weight = $new_praise_no_weight + $user_info['praise_weight'];
        $weight_balance = $weight_balance +$user_info['praise_weight'];
        // 计算当前赞踩能量为负时设置0
        if($weight_balance < 0){
            $weight_balance = 0;
        }
		...
		if($user_reputation > 0){
            ...
            // 获取组信息
            $group_info = $this->model('account')->get_user_group_by_id($user_info['reputation_group']);

            $new_praise_no_weight = intval($user_reputation * $group_info['reputation_praise'] / 100);
            $weight_balance = $user_info['weight_balance'];
            $auto_increase = get_setting("integral_system_config_praise_oppose_auto_increase");
            // 判断剩余赞踩能量小于赞踩能量
            if ($weight_balance < $new_praise_no_weight &&$new_praise_no_weight - $weight_balance > $auto_increase) {
                $weight_balance += $auto_increase;
            }else{
                $weight_balance = $new_praise_no_weight;
            }
            ...
            $this->model('account')->update_users_fields(array(
                'praise_no_weight'=>$new_praise_no_weight,
                'weight_balance'=>$weight_balance
            ), $uid);
        }


        RETURN round($user_reputation);
	}

	public function calculate($start = 0, $limit = 100)
	{
		if ($users_list = $this->query_all('SELECT uid FROM ' . get_table('users') . ' where online_time > 5  ORDER BY reputation desc', intval($start) . ',' . intval($limit)))
		{
            //$avg_reputation = $this->fetch_one("users", "avg(reputation)");
