...
class weixin_class extends AWS_MODEL
{
    ...
    public function add_articles_to_mpnews($article_ids)
    {
        ...
        foreach ($articles_info AS $article_info)
        {
            ...
            $this->mpnews['articles'][] = array(
                                                ...
                                                'content' => nl2br(FORMAT::parse_bbcode($article_info['message'])),
                                                'show_cover_pic' => '0'
                                            );
    ...
    public function add_questions_to_mpnews($question_ids)
    {
        ...
        foreach ($questions_info AS $question_info)
        {
            ...
            $this->mpnews['articles'][] = array(
                                                ...
                                                'content' => nl2br(FORMAT::parse_bbcode($question_info['question_detail'])),
                                                'show_cover_pic' => '0'
                                            );
    ...
    public function generate_qrcode($url = ''){
        require_once 'openid/weixin/phpqrcode.php';
     ...
     * @return string
     */
    public function generate_qrcode_img($url = '',$uid = ''){
        require_once 'openid/weixin/phpqrcode.php';
...
//        $dir = get_setting('upload_dir')."/uploads/poster/qrcode";
//        $dir = $_SERVER['DOCUMENT_ROOT']."/../uploads/poster/qrcode"; // 47环境路径
        ...
        try{
            $QR = QRcode::png($value,$url, $errorCorrectionLevel, $matrixPointSize, 2); //已经生成的原始二维码图
            $qr_img = $url;
            $logo = G_STATIC_URL . '/common/qrcode_logo.png';//准备好的logo图片
    ...
    public function createPoster($config=array(), $filename=""){
        ...
        $backgroundFun = 'imagecreatefrom'.image_type_to_extension($backgroundInfo[2], false);  //由文件或 URL 创建一个新图象。
        $background = $backgroundFun($background);
        $backgroundWidth = imagesx($background);  //背景宽度
        $backgroundHeight = imagesy($background);  //背景高度
        $imageRes = imageCreatetruecolor($backgroundWidth,$backgroundHeight); //新建一个真彩色图像
        $color = imagecolorallocate($imageRes, 0, 0, 0);    //为一幅图像分配颜色
        imagefill($imageRes, 0, 0, $color);     //颜色区域填充
...
//                list($R,$G,$B) =explode(',', $val['fontColor']);
                ...
                imagettftext($imageRes,$val['fontSize'],$val['angle'],$val['left'],$val['top'],$fontColor,$val['fontPath'],$val['text']); //字体向图像写入文本
            }
        }
